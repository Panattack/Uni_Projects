...
def board(n):
    ...
    return { (x, y):False for x in range(n) for y in range(n) }
...
def get_iterator(board):
    ...
    l.sort()
    return iter(l)'''
    #xreiazomai to megethos gia na ftiajw ton generator
    size = get_size(board)
    # tha prepei na ftiajw generator comprehension 
    return ( ((x,y), board[x,y]) for x in range(size) for y in range(size))
#---------------------- Askisi 5 ------------------------
def print_board(board):
    ...
    for x in board:
        if n%k != 0:
            if not(is_alive(board, x)):print(chr(11036), end='')
            else:print(chr(11035), end='')
        else:
            if not(is_alive(board, x)):print(chr(11036))
            else:print(chr(11035))
        n+=1
#---------------------- Askisi 6 ------------------------
def neighbors(p):
    """Geitonika kelia.133669
    ...
    """
    """GRAPSTE TON KWDIKA SAS EDW."""
    return {(p[0], p[1]+1), (p[0], p[1]-1), (p[0]+1, p[1]), (p[0]+1, p[1]-1), (p[0]+1,p[1]+1), (p[0]-1,p[1]), (p[0]-1,p[1]-1), (p[0]-1, p[1]+1)}
...
def place_blinker(board, p = (0,0)):
    ...
    li=board.keys()
    for x in li:
        if x[1]==p[1] and x[0]-p[0]>=0 and x[0]-p[0]<=2:
            board[x]=True
...
def place_glider(board, p = (0,0)):
    ...
    li = board.keys()
    for x in li:
        if x[1] == p[1] + 2 and (x[0] == p[0] or x[0] == p[0]+1 or x[0] == p[0] + 2):
            board[x] = True
        elif x[0] == p[0] + 2 and x[1] == p[1]+1:
            board[x] = True
        elif x[0] == p[0]+1 and p[1] == x[1]:
            board[x] = True
#---------------------- Askisi 8 ------------------------
def tick(board):
    ...
    for x in board:
        ...
        if not(board[x]):
            ...
            for i in s:
                if board[i]:
                    metritis +=1
            if metritis == 0 or metritis == 1:
                flag = False
            elif metritis == 2 or metritis == 3:
                flag = True
            else:
                flag = False
        board2[x] = flag
